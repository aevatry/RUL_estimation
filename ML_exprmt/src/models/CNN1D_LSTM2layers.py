...
class CNN1D_LSTM2layers(nn.Module):
...
class Spatial_LSTM_block (nn.Module):
    ...
    def __init__(self, pyramid_bins:list, entry_channels:int, _mode:str):
        ...
        self.LSTMcell1 = nn.ModuleList([])
        self.LSTMcell2 = nn.ModuleList([])
        ...
        for _ in pyramid_bins:
            ...
            self.LSTMcell1.append(nn.LSTMCell(input_size=self.conv_filters, hidden_size= self.hidden_layers))
            self.LSTMcell2.append(nn.LSTMCell(input_size=self.hidden_layers, hidden_size= self.hidden_layers))
    ...
    def forward(self, X):
        ...
        for i, bin_size in enumerate(self.pyramid_bins):
            ...
            h_t2 = torch.zeros(X_conv.shape[0], self.hidden_layers)
            c_t2 = torch.zeros(X_conv.shape[0], self.hidden_layers)

            for index in range(X_conv.shape[-1]):

                conv_elmt = X_conv[:,:, index]
                h_t, c_t = self.LSTMcell1[i](conv_elmt, (h_t, c_t))
                h_t2, c_t2 = self.LSTMcell2[i](h_t, (h_t2, c_t2))
            
            Xout = self.LSTMlin[i](h_t2)
            out+=[Xout]
